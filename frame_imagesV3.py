...
def pasting(original_image):
    ...
    design = PIL.Image.open(os.path.join(os.getcwd(), 'yinyang.png'))
    ...
    if use_decorative_frame: 
       # frame_pic = PIL.Image.open(os.path.join(os.getcwd(), 'frame.jpg'))
        custom_img =design.resize((result.width/4,result.height/6))
        
        result.paste(custom_img, (result.width-67,result.height-50)) # These are the coordinates you need to change
    return result
...
def paste_all_images(directory=None):
    ...
    # Create a new directory 'modified'
    new_directory = os.path.join(directory, 'emblem')
    try:
        os.mkdir(new_directory)
    except OSError:
        pass # if the directory already exists, proceed  
